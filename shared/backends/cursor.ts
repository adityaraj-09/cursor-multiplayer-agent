...
const EMPTY_UNKNOWN = new Set(["", "[object Object]", "undefined", "null"]);

function isEmptyUnknown(text: string): boolean {
  return EMPTY_UNKNOWN.has(text.trim());
}

/**
 * Turn unknown thrown values / CLI payloads into a user-visible string.
 * Never returns "[object Object]". Lives here so Next (extensionless) and
 * Node ESM (`*.js`) both resolve it without a new shared module.
 */
export function stringifyUnknown(
  ...
  if (typeof value === "string") {
    return isEmptyUnknown(value) ? fallback : value;
  }
  ...
  if (value instanceof Error) {
    ...
    if (msg && !isEmptyUnknown(msg)) return msg;
  ...
  if (Array.isArray(value)) {
    const parts = value
      ...
      .filter((part) => part && !isEmptyUnknown(part));
  ...
  if (typeof value === "object") {
    ...
    for (const key of [
      ...
      if (inner && !isEmptyUnknown(inner)) return inner;
    }
    try {
      const json = JSON.stringify(value);
      if (json && json !== "{}" && json !== "[]" && !isEmptyUnknown(json)) {
...
export function errorMessage(err: unknown, fallback = "Unknown error"): string {
  ...
  return text && !isEmptyUnknown(text) ? text : fallback;
}
