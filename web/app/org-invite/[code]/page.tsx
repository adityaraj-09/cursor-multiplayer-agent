...
import { joinOrgViaInvite } from "../../../lib/api";
...
export default function OrgInvitePage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.replace(
        `/login?redirect=${encodeURIComponent(`/org-invite/${code}`)}`,
      );
      return;
    }
    ...
    joinOrgViaInvite(code)
      .then((org) => {
        if (cancelled) return;
        writeSelectedWorkspace(org.id);
        setStatus("success");
        router.replace(
          `/dashboard?notice=${encodeURIComponent(`Joined team “${org.name}”`)}`,
        );
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  return (
    <div className="min-h-screen bg-[#141414] flex items-center justify-center px-4">
      <div className="text-center">
        {status === "loading" && (
          <p className="text-[#a0a0a0] text-[14px]">Joining team…</p>
        )}
        {status === "error" && (
          <>
